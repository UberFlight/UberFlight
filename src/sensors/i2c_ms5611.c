...
//    delay(10); // No idea how long the chip takes to power-up, but let's make it 10ms
    ...
    ack = i2cRead(MS5611_I2C, MS5611_ADDR, CMD_PROM_RD, 1, &sig);
    if (!ack)
        return false;
...
static void ms5611_reset(void)
{
    i2cWrite(MS5611_I2C, MS5611_ADDR, CMD_RESET, 1);
...
{
    uint8_t rxbuf[2] = { 0, 0 };
    i2cRead(MS5611_I2C, MS5611_ADDR, CMD_PROM_RD + coef_num * 2, 2, rxbuf); // send PROM READ command
...
{
    uint8_t rxbuf[3];
    i2cRead(MS5611_I2C, MS5611_ADDR, CMD_ADC_READ, 3, rxbuf); // read ADC
    return (rxbuf[0] << 16) | (rxbuf[1] << 8) | rxbuf[2];
}

static void ms5611_start_ut(void)
{
    i2cWrite(MS5611_I2C, MS5611_ADDR, CMD_ADC_CONV + CMD_ADC_D2 + ms5611_osr, 1); // D2 (temperature) conversion start!
}
...
static void ms5611_start_up(void)
{
    i2cWrite(MS5611_I2C, MS5611_ADDR, CMD_ADC_CONV + CMD_ADC_D1 + ms5611_osr, 1); // D1 (pressure) conversion start!
}
