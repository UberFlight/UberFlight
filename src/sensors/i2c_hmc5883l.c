...
bool hmc5883lDetectI2c(sensor_t *mag)
{
    ...
    ack = i2cRead(HMC5883L_I2C, MAG_ADDRESS, 0x0A, 1, &sig);
    if (!ack || sig != 'H')
        return false;
...
void hmc5883lInit(sensor_align_e align)
{
...
#endif
    delay(50);
    i2cWrite(HMC5883L_I2C, MAG_ADDRESS, HMC58X3_R_CONFA, 0x010 + HMC_POS_BIAS);   // Reg A DOR = 0x010 + MS1, MS0 set to pos bias
    // Note that the  very first measurement after a gain change maintains the same gain as the previous setting.
    // The new gain setting is effective from the second measurement and on.
    i2cWrite(HMC5883L_I2C, MAG_ADDRESS, HMC58X3_R_CONFB, 0x60); // Set the Gain to 2.5Ga (7:5->011)
    delay(100);
    hmc5883lRead(magADC);

    for (i = 0; i < 10; i++) {  // Collect 10 samples
        i2cWrite(HMC5883L_I2C, MAG_ADDRESS, HMC58X3_R_MODE, 1);
        delay(50);
    ...
    i2cWrite(HMC5883L_I2C, MAG_ADDRESS, HMC58X3_R_CONFA, 0x010 + HMC_NEG_BIAS);   // Reg A DOR = 0x010 + MS1, MS0 set to negative bias.
    for (i = 0; i < 10; i++) {
        i2cWrite(HMC5883L_I2C, MAG_ADDRESS, HMC58X3_R_MODE, 1);
        delay(50);
        hmc5883lRead(magADC);               // Get the raw values in case the scales have already been changed.
    ...
    // leave test mode
    i2cWrite(HMC5883L_I2C, MAG_ADDRESS, HMC58X3_R_CONFA, 0x70);   // Configuration Register A  -- 0 11 100 00  num samples: 8 ; output rate: 15Hz ; normal measurement mode
    i2cWrite(HMC5883L_I2C, MAG_ADDRESS, HMC58X3_R_CONFB, 0x20);   // Configuration Register B  -- 001 00000    configuration gain 1.3Ga
    i2cWrite(HMC5883L_I2C, MAG_ADDRESS, HMC58X3_R_MODE, 0x00);    // Mode register             -- 000000 00    continuous Conversion Mode
    delay(100);
...
bool hmc5883lRead(int16_t *magData)
{
    ...
    i2cRead(HMC5883L_I2C, MAG_ADDRESS, MAG_DATA_REGISTER, 6, buf);
