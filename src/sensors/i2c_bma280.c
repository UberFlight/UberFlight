...
bool bma280Detect(sensor_t *acc)
{
    ...
    ack = i2cRead(BMA280_I2C, BMA280_ADDRESS, 0x00, 1, &sig);
    if (!ack || sig != 0xFB)
        return false;
...
static void bma280Init(sensor_align_e align)
{
    i2cWrite(BMA280_I2C, BMA280_ADDRESS, BMA280_PMU_RANGE, 0x08); // +-8g range
    i2cWrite(BMA280_I2C, BMA280_ADDRESS, BMA280_PMU_BW, 0x0E); // 500Hz BW
...
static bool bma280Read(int16_t *accelData)
{
    ...
    i2cRead(BMA280_I2C, BMA280_ADDRESS, BMA280_ACC_X_LSB, 6, buf);

    // Data format is lsb<5:0><crap><new_data_bit> | msb<13:6>
    data[X] = (int16_t)((buf[0] >> 2) + (buf[1] << 8));
    data[Y] = (int16_t)((buf[2] >> 2) + (buf[3] << 8));
    data[Z] = (int16_t)((buf[4] >> 2) + (buf[5] << 8));
