...
bool ledringDetect(void)
{
    ...
    ack = i2cWrite(LEDRING_I2C, LED_RING_ADDRESS, 0xFF, sig);
    if (!ack)
        return false;
    return true;
}
...
void ledringState(void)
{
    ...
    if (state == 0) {
        ...
        i2cWriteBuffer(LEDRING_I2C, LED_RING_ADDRESS, 0xFF, 2, b);
        state = 1;
    } else if (state == 1) {
        b[0] = 'y';
        b[1] = constrain(angle[ROLL] / 10 + 90, 0, 180);
        b[2] = constrain(angle[PITCH] / 10 + 90, 0, 180);
        i2cWriteBuffer(LEDRING_I2C, LED_RING_ADDRESS, 0xFF, 3, b);
        state = 2;
    } else if (state == 2) {
